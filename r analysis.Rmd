...
summary_datas <- data.frame(
  ...
  count = c(378.407, 48.480)  # these counts should be in thousands as per your data
)
view(summary_datas)
...
ggplot(data = summary_datas) +
  ...
  scale_y_continuous(
    limits = c(0, 100),  # Adjusted to 100% to include all data points
    labels = percent_format(accuracy = 1, scale = 1/1),
    breaks = seq(0, 100, 10)  # Adjust breaks to every 10%
  )
```
